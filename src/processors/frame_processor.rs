...
use crate::{Frame, FrameType, StartFrame, ErrorFrame, CancelFrame};
use crate::task_manager::{TaskManager, TaskHandle, WatchdogConfig};
...
pub trait Observer: Send + Sync {
    async fn on_process_frame(&self, processor_name: &str, frame: &dyn Frame, direction: FrameDirection);
    async fn on_push_frame(&self, from: &str, to: &str, frame: &dyn Frame, direction: FrameDirection);
}
...
pub trait FrameProcessorTrait: Send + Sync {
    async fn process_frame(&mut self, frame: FrameType, direction: FrameDirection) -> Result<(), String>;
    fn name(&self) -> &str;
...
impl FrameProcessor {
        ...
        Fut: std::future::Future<Output = ()> + Send + 'static,
    {
        ...
        self.task_manager.create_task(task_name, future, None).await
    ...
    pub async fn cancel_task(&self, task: &TaskHandle, timeout: Option<Duration>) -> Result<(), String> {
        self.task_manager.cancel_task(task, timeout).await
            .map_err(|e| format!("Failed to cancel task: {:?}", e))
    }
    
    /// Setup method for processors
    pub async fn setup_with_task_manager(&mut self, setup: FrameProcessorSetup) -> Result<(), String> {
    ...
    pub async fn queue_frame(
        ...
        if self.enable_direct_mode {
            ...
            if let Some(tx) = &self.input_tx {
                tx.send(item).map_err(|e| format!("Failed to queue frame: {}", e))?;
            }
    ...
    async fn process_frame_impl(&mut self, frame: FrameType, direction: FrameDirection) -> Result<(), String> {
        ...
        if let Some(observer) = &self.observer {
            ...
            log::debug!("{}: Observer notification - frame: {}, direction: {:?}, timestamp: {}", 
                self.name, frame.name(), direction, timestamp);
            
            observer.on_process_frame(&self.name, &frame, direction).await;
        }
        ...
        match &frame {
            ...
            FrameType::Error(error_frame) => {
                log::debug!("{}: Processing ErrorFrame: {}", self.name, error_frame.error);
            ...
            _ => {
                ...
                if frame.name() == "StartInterruptionFrame" {
                    ...
                    if let Some(_processor_impl) = &self.processor_impl {
                        ...
                        log::debug!("{}: Would call custom processor for frame {}", self.name, frame.name());
                    }
                }
            }
        }
    ...
    pub async fn pause_processing_system_frames(&self) {
        ...
        self.should_block_system_frames.store(true, Ordering::Relaxed);
    }
    ...
    pub async fn push_error(&self, error: ErrorFrame) -> Result<(), String> {
        self.push_frame(FrameType::Error(error), FrameDirection::Upstream).await
    }
    ...
    async fn internal_push_frame(&self, frame: FrameType, direction: FrameDirection) -> Result<(), String> {
        match direction {
            FrameDirection::Downstream => {
                if let Some(next) = &self.next {
                    ...
                    if let Some(observer) = &self.observer {
                        ...
                        observer.on_push_frame(&self.name, &next_guard.name, &frame, direction).await;
                        drop(next_guard);
                    }
            ...
            FrameDirection::Upstream => {
                if let Some(prev) = &self.prev {
                    ...
                    if let Some(observer) = &self.observer {
                        ...
                        observer.on_push_frame(&self.name, &prev_guard.name, &frame, direction).await;
                        drop(prev_guard);
                    }
    ...
    fn check_started(&self, frame: &dyn Frame) -> bool {
        ...
        if !started {
            log::error!("{}: Trying to process {} but StartFrame not received yet", self, frame.name());
        }
        started
    }
    ...
    async fn cancel_input_task(&mut self) {
        if let Some(task_handle) = self.input_frame_task.take() {
            let _ = self.task_manager.cancel_task(&task_handle, Some(Duration::from_secs(5))).await;
        }
        self.input_tx = None;
    }
                        ...
                        Box::pin(async move {
                            ...
                            while let Some(item) = rx.recv().await {
                                ...
                                match &item.frame {
                                    ...
                                    FrameType::Error(error_frame) => {
                                        log::debug!("{}: Processing ErrorFrame in process task: {}", name, error_frame.error);
                                    }
                                    _ => {
                                        log::debug!("{}: Processing frame {} in process task", name, item.frame.name());
                                    }
                                }
